...
var leftComsZone = {
    ...
    showTemplates: function (option) {
        if (option.data) {
            ...
            this.templateInfo[option.template].more = true;
...
//                $(".tab-join-section[data-id='" + data.notice.infos[i]['k'] + "']").find("input").val(data.notice.infos[i]['v']);
    ...
    showToolbar: function (target) {
        var _html = "<div id='item_toolbar' class='item-toolbar' style='width: 300px;'>" +
            "<div onclick='previewZone.editCom(this);' class='i-edit'>" +
            "<span class='fa fa-edit'></span>" +
            "<span>编辑</span>" +
            "</div>" +
            "<div onclick='previewZone.copyCom(this);' class='i-copy'>" +
            ...
            "<span>删除</span>" +
            "</div>" +
            "<div onclick=" + "previewZone.addPlaceholder('before');" + " style='border-right: 1px solid #dedede;border-left: 1px solid #dedede;'>" +
            "<span class='fa fa-angle-double-up'></span>" +
            "<span>前空行</span>" +
            "</div>" +
            "<div onclick=" + "previewZone.addPlaceholder('after');" + " >" +
        ...
        if (_type === 'rtext') { //正常组件走ue编辑器编辑
            ...
            if ($(_this).hasClass('i-edit')) {
                ...
                myVideo.open({
                    ...
                    title: '视频编辑',
                    from: '',
                    callback: {obj: previewZone, func: previewZone.editVideo, params: {type: _type}}
                });
            } else {
    ...
    copyCom:function(_this){
        var $thisComItem = $(_this).parents('.com-item');
        var $comHtml = $thisComItem.clone().removeClass('selected').removeAttr('id');
        //复制的html移除掉toolbar元素
        $comHtml.find('#item_toolbar').remove();
        $thisComItem.after($comHtml[0]);
    },    
    //删除组件
    ...
    openSingleEditor: function (_src, callback, _option) {
        ...
        var _scale = "<div style='display: inline-block;position: absolute;left: 58px;top: 15px;color: grey'>" +
            "<span onclick='imgEdit.scale.fangda();' id='scale_fangda' class='fa fa-search-plus fa-lg hover-f66926'></span>" +
            "<span>&nbsp;&nbsp;&nbsp;</span>" +
            "<span onclick='imgEdit.scale.suoxiao();' id='scale_suoxiao' class='fa fa-search-minus fa-lg hover-f66926'></span>" +
            "</div>";
        layer.open({
            type: 1,
            title: false,
            ...
            success: function (layero, index) {
                ...
                $(layero).find('.layui-layer-btn').css('position', 'relative');
                $(layero).find('.layui-layer-btn').append(_scale);
                //初始化单图编辑的相关方法
    ...
    openUEditor: function (_type) {
        ...
        layer.open({
                    ...
                    UE.registerUI('hdj', function (editor, uiName) {
                        var btn = new UE.ui.Button({
                            ...
                            onclick: function () {
                                ...
                                myGallery.open({
                                    from: layero,
                                    multiple: _this.fastAppend,
                                    change: false,
                                    layer: "#lay-myimg-box",
                                    callback: {obj: previewZone, func: previewZone.useMyImage, params: {type: '2'}}
                                });
                            }
                        });
                        return btn;
            ...
            btn2: function () {  //确定按钮
                var keepLink = $('#keep_link').prop("checked");
                if(!keepLink || keepLink == 'false'){
    ...
    //保存超链接
    insertLink:function (lay) {
        var _html = "<div class='keep-link'>" +
            "<span>保留超链接 &nbsp</span>" +
            "<input id='keep_link' type='checkbox'>" +
            "</div>";
        $(lay).append(_html);
    },
    //ue中打开我的图库
    ...
    //插入空行
    addPlaceholder: function (_type) {
        var _html = "<section class='com-item'><section><p style='padding: 10px 0;'></p></section></section>"
        if (_type == 'after') {
            $(previewZone.item).after(_html);
        } else if (_type == 'before') {
            $(previewZone.item).before(_html);
        }
    }
};
    ...
    toAddCom: function (el) {
        ...
        if (_type === "img") {
            myGallery.open({
                ...
                change: false,
                layer: "#lay-myimg-box",
                callback: {obj: topZone, func: topZone.toShowPreview, params: {type: _type}}
            });
        } else if (_type === "video") {
