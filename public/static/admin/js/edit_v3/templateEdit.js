...
var gLeftShowContainerId = '';  //当前左侧展示的来源容器ID
        ...
        publishRequest.publish(basicData, function(result){
            if(checkAjaxResult(result)){
                ...
                if(type == 1){
                    ...
                    layerPreview("/Activity/previewDetail?id="+result.data.id,result.data.qrcodeUrl);
...
//      layer.msg('我的视频正在开发调试...')
    ...
    changeBackground:function () {
		...
		$(".com-name-ul.left-tab-ul li[data-tag='12']").trigger('click');
        $('.com-container').css('height','calc(100% - 90px)');
    },
...
function getPostData(option){
    ...
    var detailHtml = pageOption.getForMobileDetail();
    postData.details = detailHtml;
...
function toPreview(){
    ...
    var postData = {};
    postData.details = pageOption.getForMobileDetail();
    ...
    open:function(option){
        ...
        layer.open({
            ...
            success:function(layero, index){
                editRequest.getTemplateTagsGroup({type:3},function(res){
                    var tags = res.data;
...
var material = {
    open:function(option){
        var _that = this;
        var $comHtml = $(option).parents('.com-item').clone().removeClass('selected').removeAttr('id');
        //收藏的html移除掉toolbar元素
        $comHtml.find('#item_toolbar').remove();
        var content = $comHtml[0].outerHTML;
        layer.open({
            type: 1,
            title: "保存素材",
            content: $("#lay-material-set"),
            skin: 'layer-title',
            area: ['auto', 'auto'],
            btn: ["保存", "取消"],
            success:function(layero, index){
            ...
            yes: function (index, layer) {
                var name = $.trim($("#lay_material_title").val());
                var type  = $("#lay_material_type").val();
                var html = content;
                var tags = $("#tags_select").attr("data-id");
                var status = $("#lay_material_status").attr("data-value");
                $.ajax({
                    type: "post",
                    url: "/Material/toAdd",
                    data:{
