...
var banner = {
    ...
    triggerClick:function() {
        $('#' + banner.config.fileInputId).trigger('click');
    },
    //图片上传预览    IE是用了滤镜。
...
//                div.innerHTML = "<div id=divhead style='width:" + rect.width + "px;height:" + rect.height + "px;margin-top:" + rect.top + "px;" + sFilter + src + "\"'></div>";
    ...
    crop:function(option) {
        ...
        jcropLayer.open(_option,{
            ...
            title: banner.config.layerTitle
        },{
            aspectRatio: banner.config.aspectRatio || null,
            allowSelect:true,
    ...
    upload:function(option){
        ...
        qiniu.getUploadToken(function(result){
            ...
            var key = banner.config.urlKey + new Date().getTime() + parseInt(Math.random() * 1000) + '.jpg';
    ...
    uploadCallback:function(result,extend){
        if(result.state === 1){
            ...
            if(banner.config.outInputId != ''){
                $('#' + banner.config.outInputId).attr("value", _url);
            }
        }else{
            console.log("上传图片失败");
        }
