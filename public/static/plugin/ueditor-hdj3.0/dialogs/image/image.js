    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
                //这里可以通过data对象添加POST参数 huodongju
                ...
                data.key = "activity_detail/" + year + month + "/web-" + new Date().getTime() + Math.floor(Math.random() * 1000) + ".jpg";
            });
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                json.original = '';
                var $file = $('#' + file.id);
                //huodongju 上传成功后，添加到用户图库中，前端html不实时更新
                try {
                    if (json.state == 'SUCCESS') {
                        $.ajax({
                            ...
                            data:{key:json.url},
                            async:true,
                            url: "http://" + window.location.host + "/" + 'Studio/addUserImage/',
                            success:function(res){
//                                console.log(res);
                            }
                        });                                                
