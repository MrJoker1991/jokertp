...
class WechatApi
{
    ...
    public function __construct($config)
    {
        ...
        includeExtend("WeChatSDK.Utils.HttpCurl");
        includeExtend("WeChatSDK.Utils.Error");
        includeExtend("WeChatSDK.Utils.SHA1");
        includeExtend("WeChatSDK.Utils.Xml");
    }
    ...
    public function new_access_token()
    {
        ...
        if ($res === false) {
            debug_log('Http Get AccessToken Error.');
            return false;
        }

        // 异常处理: access_token获取失败
        if (!isset($res->access_token)) {
            debug_log('Get AccessToken Error: ' . json_encode($res));
            return false;
        }
        $res->expires_in += time();
        return $res;
    ...
    public function get_access_token()
    {
        ...
        if ($this->get_access_token_diy !== false) {
            ...
            $token = call_user_func($this->get_access_token_diy, array($this->appId));
            if ($token) {
//                $token = json_decode($token);
            }
        } else {
            // 异常处理: 获取access_token方法未定义
            debug_log('Not set get_tokenDiy method, AccessToken will be refreshed each time.');
        }
        // 验证AccessToken是否有效        
        if (!$this->valid_access_token($token)) {
            ...
            if ($this->save_access_token_diy !== false) {
                ...
                call_user_func($this->save_access_token_diy, array($this->appId, $token));
            } else {
                // 异常处理: 保存access_token方法未定义
                debug_log('Not set saveTokenDiy method, AccessToken will be refreshed each time.');
            }
        }
     ...
     * @return bool
     */
    public function afresh_new_access_token()
    {
        $token = $this->new_access_token();
        if ($token === false) {
            return false;
    ...
    public function send($openid, $msg)
    {
        ...
        } elseif ($res->errcode == Error::CODE_ACCESS_TOKEN_ERROR) {
            $token = $this->afresh_new_access_token();
            if ($token) {
                $this->send($openid, $msg);
            } else {
...
//        }
     ...
     * ```
     */
    public function create_menu($json, $access_token = '')
    {
        $url = self::API_DOMAIN . 'cgi-bin/menu/create?access_token=' . ($access_token ? $access_token : $this->get_access_token());
...
//        }
    ...
    {
        return $ticket && isset($ticket->expires_in) && ($ticket->expires_in > time());
    }
    ...
    public function get_jsapi_ticket()
    {
        ...
        if ($this->get_jsapi_ticket_diy !== false) {
            ...
            $ticket = call_user_func($this->get_jsapi_ticket_diy, array($this->appId));
            if ($ticket) {
        ...
        if (!$this->valid_jsapi_ticket($ticket)) {
            ...
            if ($this->save_jsapi_ticket_diy !== false) {
                // 用户自定义保存AccessToken方法
                call_user_func($this->save_jsapi_ticket_diy, array($this->appId, $ticket));
            } else {
                // 异常处理: 保存access_token方法未定义
     ...
     * @int $expire_seconds 可选：该二维码有效时间，以秒为单位。 最大不超过604800（即7天）
     * @int $is_limit 可选：默认为临时二维码，填写该项为永久二维码
     *
     * @return array(err, data)
     * - `err`, 调用失败时得到的异常
     * - `res`, 调用正常时得到的对象
     ...
     * ```
     */
    public function create_qrcode($scene_id, $expire_seconds = 0, $is_limit = 0)
    {
        ...
        $action_name = $is_limit == 0 ? 'QR_STR_SCENE' : 'QR_LIMIT_SCENE';
...
//            return array(null, $res);
            ...
            $token = $this->afresh_new_access_token();
            if ($token) {
                $this->create_qrcode($scene_id, $expire_seconds, $is_limit);
            } else {
                return false;
            }
        } else {
...
//            return array($res, null);
    ...
    public function get_user_info($open_id, $lang = '')
    {
        ...
        if (isset($res->openid)) {
            ...
            $token = $this->afresh_new_access_token();
            if ($token) {
                $this->get_user_info($open_id, $lang);
            } else {
                return false;
            }
        } else {
     ...
     * ```
     *
     * @param string $code 通过code换取网页授权access_token
     * @param $scope `get_authorize_url`时使用的授权类型
     * @param string $lang 可选，返回国家地区语言版本，zh_CN 简体，zh_TW 繁体，en 英语
     *
     * @return array|object
     */
    public function get_userinfo_by_authorize($code, $scope, $lang = 'zh_CN')
    {
        if (!empty($code)) {
    ...
    {
        $url = self::API_DOMAIN . 'cgi-bin/message/template/send?access_token=' . ($access_token ? $access_token : $this->get_access_token());
        ...
        if ($res->errcode == 0) {
            ...
            debug_log('WeChat: send_template_message error ' . $res->errcode . ' msg:' . $res->errmsg);
            return false;
        }
    }
}
